...
let ResetPasswordService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    if (!userToken) {
      throw new _AppError.default('User token does not exist.', 400);
    }

    const user = await this.usersRepository.findById(userToken.user_id);

    if (!user) {
      throw new _AppError.default('User does not exist.', 400);
    }
    ...
    if ((0, _dateFns.differenceInHours)(Date.now(), tokenCreatedAt) > 2) {
      throw new _AppError.default('Token has expired.', 400);
    }
