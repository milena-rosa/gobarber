...
let CreateAppointmentService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    if ((0, _dateFns.isBefore)(appointmentDate, Date.now())) {
      throw new _AppError.default('This appointment has already passed.', 400);
    }

    if (user_id === provider_id) {
      throw new _AppError.default('It is not possible to make an appointment with yourself.', 400);
    }

    if ((0, _dateFns.getHours)(appointmentDate) < 8 || (0, _dateFns.getHours)(appointmentDate) > 17) {
      throw new _AppError.default('It is not possible to make an appointment before 8am and after 5pm.', 400);
    }

    const foundAppointmentInSameDate = await this.appointmentsRepository.findByDate(appointmentDate, provider_id);

    if (foundAppointmentInSameDate) {
      throw new _AppError.default('This appointment has already been taken.', 400);
    }
