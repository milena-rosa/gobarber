...
let UpdateProfileService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    const existsUserWithUpdatedEmail = await this.usersRepository.findByEmail(email);

    if (existsUserWithUpdatedEmail && existsUserWithUpdatedEmail.id !== user_id) {
      throw new _AppError.default('This email has already been taken.', 400);
    }
    ...
    if (password && !old_password) {
      throw new _AppError.default('The old password has to be set.', 400);
    }
    ...
    if (password && old_password) {
      ...
      if (!checkOldPassword) {
        throw new _AppError.default('The old password does not match.', 400);
      }
